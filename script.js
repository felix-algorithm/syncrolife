...
const githubReleaseApiUrl = "https://api.github.com/repos/felix-algorithm/syncrolife.com/releases/tags/v1.0.0";
const githubAssetName = "app-release.apk";
const visitCounterEndpoint = "";
const localVisitStorageKey = "syncro-life-local-visits";
...
function updateLocalVisits() {
  const currentVisits = Number(localStorage.getItem(localVisitStorageKey) || 0) + 1;
  localStorage.setItem(localVisitStorageKey, String(currentVisits));
  setStatValue(visitCount, currentVisits);
}

async function loadGlobalVisits() {
  if (!visitCounterEndpoint) {
    updateLocalVisits();
    return;
  }
  ...
  try {
    ...
    const visits = data.value ?? data.count ?? data.visits;
    setStatValue(visitCount, visits);
    if (visitLabel) {
      visitLabel.textContent = "Lượt truy cập";
    }
  } catch {
    updateLocalVisits();
  }
...
async function loadGithubDownloads() {
  ...
  try {
    const response = await fetch(githubReleaseApiUrl, {
      cache: "no-store",
      signal: controller.signal
    });
    if (!response.ok) throw new Error("GitHub release request failed");
    const release = await response.json();
    const targetAsset = release.assets?.find((asset) => asset.name === githubAssetName);
    const totalDownloads = targetAsset?.download_count ?? release.assets?.reduce((total, asset) => total + (asset.download_count || 0), 0) ?? 0;
    setStatValue(downloadCount, totalDownloads);
  } catch {
    downloadCount.textContent = "N/A";
  } finally {
